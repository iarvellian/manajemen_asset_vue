<template>
  <!-- Page Heading -->
  <h1 class="h3 mb-2 text-gray-800">Tambah Aset Masuk</h1>
    ...
    <div class="card-body">
      <div class="row">
        <div class="col-md-6">
          ...
          <div class="card">
            <div class="card-header">
              <h5 class="card-title">Aset Diluar</h5>
            </div>
            <div class="card-body">
              <!-- Scrollable table for the left card -->
              <div class="table-responsive">
                <table class="table table-hover table-bordered" id="tableAssetKeluar" width="100%" cellspacing="0">
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    <tr v-for="assetkeluar in assetkeluars.data" :key="assetkeluar.id_transaksi_keluar">
                      ...
                      <td>{{ assetkeluar.asset.kode_aset }}</td>
                      <td class="text-center">
                        <input type="checkbox" v-model="assetkeluar.selected" @change="handleCheckboxChange(assetkeluar)">
                      </td>
                    </tr>
                  </tbody>
                </table>
        ...
        <div class="col-md-6">
          ...
          <div class="card">
            <div class="card-header">
              <h5 class="card-title">Asset Terpilih</h5>
            </div>
            <div class="card-body">
              ...
              <div class="table-responsive">
                <table class="table">
                  ...
                  <tbody>
                    ...
                    <tr v-for="asset in selectedAssets" :key="asset.id_transaksi_keluar">
              ...
              <div class="text-center">
                <button v-if="selectedAssets.length > 0 && !showBeritaAcaraForm" type="button"
                  @click="confirmCreateAssetMasuk" class="btn btn-primary btn-sm">Simpan</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <button type="button" @click="cancelCreation" class="btn btn-danger btn-sm ml-2 mt-4">
        Kembali
      </button>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    createAssetMasuk(createBeritaAcara, showSuccessSwal = true) {
      const postData = {
    ...
    initDataTable() {
      this.$nextTick(() => {
        ...
        this.dataTable = $("#tableAssetKeluar").DataTable({
          autoWidth: false,
        });
      });
    },
